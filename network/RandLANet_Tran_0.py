...
class Network(nn.Module):
    ...
    def forward(self, end_points):
        ...
        f_sampled_i = self.random_sample(f_encoder_i, end_points['sub_idx'][0])
        features = f_sampled_i # 会传入到下一个中
        f_encoder_list.append(f_encoder_i)
        f_encoder_list.append(f_sampled_i)

        # i = 1
        f_encoder_i = self.dilated_res_blocks[1](features, end_points['xyz'][1], end_points['neigh_idx'][1])
        f_sampled_i = self.random_sample(f_encoder_i, end_points['sub_idx'][1])
        features = f_sampled_i
        f_encoder_list.append(f_sampled_i)
